...
const defaultGraph = studioData.stackGraph;
const defaultSpaces = studioData.rooms.Spaces;
...
export function RoomRelationshipGraph({ graph = defaultGraph, floor, selected, onSelect, heatmap }) {
  ...
  [1, 2, 3, 4, 5, 6].forEach(level => {
    ...
    levelNodes.forEach((node, index) => {
      ...
      positions[node.globalId] = [122 + column * (715 / Math.max(1, columns - 1)), 126 + (6 - level) * 62 + (top ? -18 : 18)];
    });
  });
  const heatColor = node => heatmap ? (node.floorLevel === floor ? (heatmap === 'daylight' ? '#f2c84b' : '#42b595') : '#d8dfdc') : roomColor(node.spaceCategory);
  return <svg className="room-graph-svg" viewBox="60 62 840 500" role="img" aria-label={`All floors 3D room relationship graph · ${floorName(floor)} highlighted`}><rect className="room-floor-frame" x="82" y="88" width="796" height="444" rx="24" />{[1, 2, 3, 4, 5, 6].map(level => <g key={level} className={`room-floor-band ${level === floor ? 'active' : ''}`}><line x1="96" y1={125 + (6 - level) * 62} x2="864" y2={125 + (6 - level) * 62} /><text x="104" y={119 + (6 - level) * 62}>{floorName(level)}/F</text></g>)}{edges.map((edge, index) => { const start = positions[edge.source]; const end = positions[edge.target]; return start && end ? <line key={`${edge.source}-${edge.target}-${index}`} className="room-edge" x1={start[0]} y1={start[1]} x2={end[0]} y2={end[1]}><title>{edge.adjacencyType || 'adjacency'} · {edge.source} to {edge.target}</title></line> : null; })}{nodes.map(node => { const position = positions[node.globalId]; const active = node.floorLevel === floor; return <g key={node.globalId} className={`room-node ${active ? 'active-floor' : ''} ${selected === node.globalId ? 'selected' : ''}`} transform={`translate(${position[0]} ${position[1]})`} role="button" tabIndex="0" onClick={() => onSelect(node.globalId)} onKeyDown={event => (event.key === 'Enter' || event.key === ' ') && onSelect(node.globalId)}><circle r={active ? 18 : 10} fill={heatColor(node)} /><text>{active ? node.globalId : node.globalId.slice(0, 4)}</text><title>{node.globalId} · {node.spaceType || node.name} · {node.spaceCategory}</title></g>; })}</svg>;
}
